...
def test_tool_scene(docker_compose, tmp_path, monkeypatch):
    ...
    p = run_tool_scene("delete", scene_uuid[:8], "--json")
    ...
    p = run_tool_scene("show", bogus, "--json")
    ...
    def create_scene_uuid():
        cp = run_tool_scene("create", "--file", str(usd_path), "--json")
...
async def test_tool_session(scene_on_server, tmp_path, monkeypatch):
    ...
    p = run_tool_session("delete", session_uuid[:8], "--json")
    deleted = json.loads(p.stdout.strip())
    ...
    p = run_tool_session("show", bogus, "--json")
    assert p.returncode != 0 and "not found" in p.stderr.lower()
    ...
    s1 = json.loads(
        run_tool_session("create", scene_uuid[:8], "--json").stdout.strip()
    )["uuid"]
    s2 = json.loads(
        run_tool_session("create", scene_uuid[:8], "--json").stdout.strip()
    )["uuid"]
    ...
    if s2[0] == s1[0]:
        ambiguous = s1[0]
        p = run_tool_session("show", ambiguous, "--json")
        assert p.returncode != 0 and "ambiguous" in p.stderr.lower()
    # cleanup sessions we created
    run_tool_session("delete", s1, "--json")
    run_tool_session("delete", s2, "--json")
